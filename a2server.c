...
void runProcess(int index, ticketNode * ticketToRun){
   ...
   printf("ticketToRun DATA: PID: %d RT: %c THREAD: %d ITERATION: %d WF: %d NR: %d\n", ticketToRun->pid, ticketToRun->requestType, ticketToRun->thread_id, ticketToRun->iteration, clientQueues[index].activeWriter, clientQueues[index].numActiveReaders);
...
void releaseClientQueue(int index, int pid, char requestType, char release){
   ...
   printf("GOT RELEASE FROM: PID: %d RT: %c RELEASE: %c WF: %d NR: %d\n", pid, requestType, release, clientQueues[index].activeWriter, clientQueues[index].numActiveReaders);
